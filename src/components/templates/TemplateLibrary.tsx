...
export const TemplateLibrary: React.FC<TemplateLibraryProps> = ({
  ...
  const getSourceDisplay = (template: EmailTemplate) => {
    // Check if it's AI-generated by multiple indicators
    const isAIGenerated = template.template_source === 'ai_generated' || 
                         template.tags?.includes('ai_generated') ||
                         template.personalization_variables?.ai_generated === true;
    ...
    switch (template.template_source) {
      case 'duplicated':
        return { 
          icon: Copy, 
          label: 'Duplicated', 
  ...
  const stats = useMemo(() => {
    ...
    // Check for AI-generated templates by source OR by AI indicators in tags/personalization
    const aiGenerated = templates.filter(t => 
      t.template_source === 'ai_generated' || 
      t.tags?.includes('ai_generated') ||
      t.personalization_variables?.ai_generated === true
    ).length;
    
    const duplicated = templates.filter(t => t.template_source === 'duplicated').length;
    
    // Manual count should exclude AI-generated templates (even if incorrectly marked as manual)
    const manual = templates.filter(t => 
      t.template_source === 'manual' && 
      !t.tags?.includes('ai_generated') && 
      !t.personalization_variables?.ai_generated
    ).length;
    
    return { total, aiGenerated, manual, duplicated };
  }, [templates]);
            ...
            <Table>
              ...
              <TableBody>
                {filteredTemplates.map((template) => {
                  const sourceDisplay = getSourceDisplay(template);
